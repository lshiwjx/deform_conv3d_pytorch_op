...
# inpu = torch.FloatTensor([[[[1.0,1,1],[2.0,2,2],[1.0,1,1]]] * c_in] * batchsize).cuda()
...
offset = torch.zeros(batchsize, g_c, out, out).type(torch.FloatTensor).cuda()
...
padding = [pad, pad]
stride = [stri, stri]
conv_offset = ConvOffset2dFunction(stride, padding, channel_per_group)
